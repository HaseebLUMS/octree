...
class EchoClient : public quic::QuicSocket::ConnectionSetupCallback,
 ...
 public:
  ...
  void readAvailable(quic::StreamId streamId) noexcept override {
    ...
    tcpBytesRcvd_ += dataRecvd.size();
    if (transport_scheme_ == tcp_p_udp_scheme && tcpBytesRcvd_ == RELIABLE_DATA_SIZE) {
  ...
  void onDatagramsAvailable() noexcept override {
    ...
    for (const auto& datagram : *res) {
      auto dataRcvd = datagram.bufQueue()
                       .front()
                       ->cloneCoalesced()
                       ->moveToFbString()
                       .toStdString();
...
//  public:
  ...
  void sendMessage(quic::StreamId id, BufQueue& data) {
    ...
    if (res.hasError()) {
      ...
      LOG(INFO) << "EchoClientTest wrote \"" << str << "\""
  ...
  std::chrono::milliseconds udpTimeout_{200};
